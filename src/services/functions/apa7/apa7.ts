...
export async function Apa7thEdition(
    url: string,
    lang: string
): Promise<Citiatoin> {
    const apaStyle = lang === "es" ? "APA 7ma edición" : "APA 7th edition";
    ...
    const info =
        lang === "es"
            ? "Estos ejemplos ilustran la estructura de una cita APA según la séptima edición."
            : "These examples illustrate the structure of an APA citation according to the seventh edition.";
    ...
    try {
        ...
        if (data) {
            ...
            const citiation_result = `${author}. (${publishedDate}). ${data.title}. ${url}`;
            ...
            return {
                ...
                citiation_result,
                textual_anatomic: textual_anatomic,
                anatomic,
            };
        } else {
...
export function Apa6thEdition(input: string): string {
    return input;
}
